...
let active = document.querySelectorAll('.one__box.active');
let remain = document.querySelector('#remain');
let important = document.querySelector('#important');
let boxs = document.querySelectorAll('.one__box');
let audio_success = document.querySelector('#audio_success');

let handleWater = () =>{
    remain.innerHTML = `${(active.length)*12.5}%`;
    if(active.length === 0)
        remain.innerHTML = '';
    remain.setAttribute('style', `height: ${(active.length)*12.5}%`);
    if(active.length === boxs.length)
    {
        important.innerHTML = '';
        audio_success.play();
        startConfetti();
    ...
    else
    {
        ...
        important.innerHTML = `${(100 - (active.length)*12.5)/12.5*250} ml`;
    }
    important.setAttribute('style', `height: ${100 - (active.length)*12.5}%`);
}
...
function checkBehind(e){
    for(var i = 0; i < boxs.length - 1; i++)
    {
        if(boxs[i] === e && boxs[i+1].classList.contains('active'))
            return boxs[i+1].classList.contains('active');
    }
    return false;
}
...
boxs.forEach(box =>{
    box.addEventListener('click', () =>{
        ...
        active = document.querySelectorAll('.one__box.active');
        handleWater(box);
    })
})
